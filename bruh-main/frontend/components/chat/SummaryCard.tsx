        ...
        <div className="bg-surface border border-borderDark rounded-2xl shadow-xl overflow-hidden mt-6 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-primary/10 px-6 py-4 flex flex-col sm:flex-row items-center justify-between border-b border-primary/20 gap-4">
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    <div>
                        ...
                        <p className="text-primary text-xs uppercase tracking-wider">Patient Medical Summary</p>
                    </div>
                </div>
            ...
            <div className="p-6 grid gap-6 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2 space-y-4">
                    <div className="flex items-center gap-2 text-textMuted text-sm font-bold uppercase tracking-wider border-b border-borderDark pb-2">
                        <AlertTriangle className="w-4 h-4 text-primary" /> Health Assessment
                    </div>
                    <div className={`p-5 rounded-xl border flex flex-col gap-4 bg-primary/5 border-primary/20`}>
                        {summary.clinical_info.assessment && (
                            <div>
                                <p className="text-sm font-medium text-textMuted uppercase tracking-wider mb-1">Assessment</p>
                                <p className="text-[15px] text-textMain leading-relaxed">{summary.clinical_info.assessment}</p>
                            </div>
                        )}

                        {summary.clinical_info.possible_causes && summary.clinical_info.possible_causes.length > 0 && (
                            <div>
                                <p className="text-sm font-medium text-textMuted uppercase tracking-wider mb-2">Possible Causes (Non-diagnostic)</p>
                                <ul className="space-y-1.5 list-disc list-inside">
                                    {summary.clinical_info.possible_causes.map((item, idx) => (
                                        <li key={idx} className="text-sm text-textMain/90">
                                            {item}
                                        </li>
                                    ))}
                                </ul>
                        ...
                        {summary.clinical_info.home_care && summary.clinical_info.home_care.length > 0 && (
                            <div>
                                <p className="text-sm font-medium text-amber-400 uppercase tracking-wider mb-2">Supportive Home Care</p>
                                <ul className="space-y-1.5">
                                    {summary.clinical_info.home_care.map((item, idx) => (
                                        <li key={idx} className="text-sm text-textMain/90 flex gap-2">
                                            <span className="text-amber-500/50 mt-0.5">•</span> {item}
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        ...
                        {summary.clinical_info.otc_guidance && summary.clinical_info.otc_guidance.length > 0 && (
                            <div>
                                <p className="text-sm font-medium text-teal-400 uppercase tracking-wider mb-2">Safe Guidance</p>
                                <ul className="space-y-1.5">
                                    {summary.clinical_info.otc_guidance.map((item, idx) => (
                                        <li key={idx} className="text-sm text-textMain/90 flex gap-2">
                                            <span className="text-teal-500/50 mt-0.5">•</span> {item}
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        ...
                        {summary.clinical_info.when_to_seek_care && summary.clinical_info.when_to_seek_care.length > 0 && (
                            <div className="pt-2 border-t border-rose-500/10">
                                <p className="text-sm font-bold text-rose-400 uppercase tracking-wider mb-2">When to Seek Immediate Care</p>
                                <ul className="space-y-1.5">
                                    {summary.clinical_info.when_to_seek_care.map((item, idx) => (
                                        <li key={idx} className="text-sm text-rose-300 flex gap-2">
            ...
            <div className="absolute left-[-9999px] top-[-9999px]">
                <div id="pdf-report-container" className="bg-white text-black p-10 w-[800px] font-sans">
                    ...
                    {/* PDF Health Assessment */}
                    <div className="mb-8">
                        <h2 className="text-lg font-bold text-gray-800 border-b border-gray-200 pb-2 mb-4 flex items-center gap-2">
                            <AlertTriangle className="w-5 h-5 text-primary" /> Health Assessment
                        </h2>
                        <div className="p-5 rounded-xl border border-gray-200 bg-gray-50 flex flex-col gap-5">
                            
                            {summary.clinical_info.assessment && (
                                <div>
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Status</p>
                                    <p className="text-sm text-gray-900 leading-relaxed">{summary.clinical_info.assessment}</p>
                                </div>
                            )}

                            {summary.clinical_info.possible_causes && summary.clinical_info.possible_causes.length > 0 && (
                                <div>
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Possible Causes (Non-diagnostic)</p>
                                    <ul className="space-y-1.5">
                                        {summary.clinical_info.possible_causes.map((item, idx) => (
                            ...
                            {summary.clinical_info.home_care && summary.clinical_info.home_care.length > 0 && (
                                <div>
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Supportive Home Care</p>
                                    <ul className="space-y-1.5">
                                        {summary.clinical_info.home_care.map((item, idx) => (
                                            <li key={idx} className="text-sm text-gray-800 flex gap-2">
                                                <span className="text-gray-400">•</span> {item}
                            ...
                            {summary.clinical_info.otc_guidance && summary.clinical_info.otc_guidance.length > 0 && (
                                <div>
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Safe Guidance</p>
                                    <ul className="space-y-1.5">
                                        {summary.clinical_info.otc_guidance.map((item, idx) => (
                                            <li key={idx} className="text-sm text-gray-800 flex gap-2">
                                                <span className="text-gray-400">•</span> {item}
                            ...
                            {summary.clinical_info.when_to_seek_care && summary.clinical_info.when_to_seek_care.length > 0 && (
                                <div>
                                    <p className="text-xs font-bold text-red-600 uppercase tracking-wider mb-2">When to Seek Immediate Care</p>
                                    <ul className="space-y-1.5">
                                        {summary.clinical_info.when_to_seek_care.map((item, idx) => (
                                            <li key={idx} className="text-sm text-red-700 flex gap-2 font-medium">
