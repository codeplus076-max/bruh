...
export default function ReportPage() {
        ...
        if (sessionId) {
            const fetchSession = async () => {
                try {
                    ...
                    console.error("Failed to fetch report session:", err);
                } finally {
                    setLoading(false);
                }
            };
    ...
    const reportTranslations = {
        en: {
            ...
            results: "Health Assessment",
            symptoms: "Clinical Symptoms",
            possible_causes: "Possible Causes (Non-diagnostic)",
        ...
        hi: {
            ...
            results: "स्वास्थ्य मूल्यांकन",
            symptoms: "नैदानिक लक्षण",
            possible_causes: "संभावित कारण (गैर-नैदानिक)",
        ...
        mr: {
            ...
            gender: "लिंग",
            lang: "अहवाल भाषा",
            results: "आरोग्य मूल्यांकन",
            symptoms: "नैदानिक लक्षणे",
            possible_causes: "संभाव्य कारणे (गैर-नैदानिक)",
    ...
    const generatePDF = () => {
        ...
        const assessmentDetails = [
            ...
            [rT.possible_causes, data.diagnosis?.possible_causes?.join(', ') || "N/A"]
        ];
                ...
                {data ? (
                    <div className="glass-panel p-8 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            ...
                            <section className="space-y-6">
                                <h2 className="text-sm font-bold text-primary uppercase tracking-widest flex items-center gap-2">
                                    <AlertCircle className="w-4 h-4" /> Health Assessment
                                </h2>
                                <div className="space-y-4">
                                    ...
                                    {(data.diagnosis?.possible_causes && data.diagnosis.possible_causes.length > 0) && (
                                    <div className="flex flex-col gap-1 pt-2">
                                        <span className="text-textMuted text-xs uppercase tracking-tighter">Possible Causes (Non-diagnostic)</span>
                                        <ul className="text-textMain text-sm leading-relaxed list-disc list-inside">
                                            {data.diagnosis.possible_causes.map((cause, idx) => (
                                                <li key={idx} className="text-secondary">{cause}</li>
                                            ))}
                                        </ul>
                                    </div>
                                    )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-borderDark pt-8">
                                { (data.diagnosis?.actions || data.guidance?.first_aid) && (data.diagnosis?.actions?.length || 0) + (data.guidance?.first_aid?.length || 0) > 0 && (
                                    <div className="space-y-3">
                                        <h3 className="text-xs font-bold text-emerald-400 uppercase tracking-widest">🩹 First Aid</h3>
                                        <ul className="space-y-2">
                                            {((data.diagnosis?.actions || data.guidance?.first_aid || []) as (string | ClinicalAction)[]).map((item, i: number) => (
                                ...
                                {(data.diagnosis?.medicines || data.guidance?.medicines) && (data.diagnosis?.medicines?.length || 0) + (data.guidance?.medicines?.length || 0) > 0 && (
                                    <div className="space-y-3">
                                        <h3 className="text-xs font-bold text-teal-400 uppercase tracking-widest">💊 OTC Medicines</h3>
                                        <div className="space-y-3">
                                            {((data.diagnosis?.medicines || data.guidance?.medicines || []) as (string | ClinicalMedicine)[]).map((med, i: number) => (
                                                <div key={i} className="bg-surfaceHighlight/30 p-3 rounded-xl border border-borderDark">
                                                    <p className="text-sm font-bold text-textMain">{typeof med === 'string' ? med : (med as ClinicalMedicine).name}</p>
                                                    <p className="text-xs text-textMuted mt-1">{typeof med === 'string' ? "" : ((med as ClinicalMedicine).guidance || (med as ClinicalMedicine).purpose)}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                { (data.diagnosis?.alerts || data.guidance?.routine) && (data.diagnosis?.alerts?.length || 0) + (data.guidance?.routine?.length || 0) > 0 && (
                                    <div className="space-y-3">
                                        <h3 className="text-xs font-bold text-rose-400 uppercase tracking-widest">🚨 Emergency Warnings</h3>
                                        <ul className="space-y-2">
                                            {((data.diagnosis?.alerts || data.guidance?.routine || []) as (string | ClinicalAction)[]).map((item, i: number) => (
                                                <li key={i} className={`text-sm flex gap-2 ${data.diagnosis?.alerts ? 'text-rose-300' : 'text-textMuted'}`}>
